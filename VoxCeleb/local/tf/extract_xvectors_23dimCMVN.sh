...
# Set up the features
feature_rspecifier="apply-cmvn-sliding-23dim --norm-vars=false --center=true --cmn-window=300 scp:${sdata}/feats.scp ark:- | select-voiced-frames ark:- scp,s,cs:${sdata}/vad.scp ark:- |"
  ...
  if ${use_gpu}; then
    for g in $(seq ${nj}); do
      ${cmd} --gpu 1 "${dir}/log/extract.${g}.log" \
        CUDA_VISIBLE_DEVICES=$((gpu_index)) local/tf/extract_embedding.py \
